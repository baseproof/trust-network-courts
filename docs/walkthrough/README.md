# Trust Network Courts — Developer Walkthrough
...
|---|---|---|
| `trust-network-courts` (this repo) | `0.0.1` | `make version` |
| `baseproof` (SDK Go module) | `v1.14.0` | `go.mod` require line |
| `ledger` (HTTP service binary) | `0.1.0` | `baseproof/ledger` repo |
...
trust-network-courts app** up: two ledgers, Postgres, real GCS,
...
- Ledger topology: `deployment/local/docker-compose.dev.yml` in
  the ledger repo
- CLI: `trust-network-courts/cmd/judicial-cli/`
