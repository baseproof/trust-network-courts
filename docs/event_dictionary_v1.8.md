---
title: Trust Network Courts — Event Dictionary
version: 1.8
status: draft
---

# Trust Network Courts: Event Dictionary

## About This Document

The trust-network-courts records every substantive event on Baseproof, an append-only, cryptographically signed log substrate — conceptually similar to Certificate Transparency logs. Entries are permanent and order-preserving; integrity is verifiable by anyone with access to the log.
...
The trust-network-courts has a three-level structural hierarchy. The rest of this dictionary reads against these definitions:

- **Network** — A single shared Baseproof log. All courts that write to a network share the same append-only sequence and the same anchor hierarchy. A network is the unit of cryptographic integrity. *Analogy: one Certificate Transparency log shared by many Certificate Authorities.*
...
The Baseproof log is the canonical, immutable, write-optimized source of truth. Search, indexing, reporting, and access-controlled views are provided by a separate aggregator microservice with its own database that consumes the log. The dictionary defines what is *written*; the aggregator defines what is *exposed*.

---

# Part 1 — Actors

The trust network courts distinguishes three roles based on cryptographic relationship to the log. Every entry traces its provenance back to a Signer, even when Filer advocates or Parties are the substantive subjects of the entry.
...
- Reframed Baseproof as the append-only signed log substrate (CT-Log-style); removed all phase / SDK / ledger / Pydantic references.
