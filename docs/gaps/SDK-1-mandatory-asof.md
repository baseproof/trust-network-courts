...
> **Depends on:** — (pairs with SDK-2; consumed by trust-network-courts JN-4)
> **Status:** Proposal / ready to implement. The `*WithTrust` API already threads
> `AsOf` through every authority path (trust-network-courts#69 / PR-C migrated the 5 JN
...
> trust-network-courts#69's open consideration ("Should `verify_authority`'s default
...
|---|---|
| Migration for existing `AsOf{}` callers | In-tree callers are JN (trust-network-courts#69 C-4); JN-4 supplies Time-of-Receipt. External callers get a compile break (intended for a zero-trust contract). |
...
- Pairs with **SDK-2** (same `AsOf` type change). Consumed by **JN-4**. Resolves
  the open `asOf`-default question in **trust-network-courts#69**.
