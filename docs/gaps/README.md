...
Format reference: `trust-network-courts#69` (PR-C tracker) — the quality bar for how
...
| **JN-1** | baseproof/trust-network-courts | Pin cross-network resolution to RootHash (`HeadByRootHash`) | Scenario 4 | SDK-2 |
| **JN-2** | baseproof/trust-network-courts | Replace static `foreignSets` with journal-backed historical sets | Scenario 2 | SDK-3, AT-1 |
| **JN-3** | baseproof/trust-network-courts | Burn-gate the 4 `VerifyCrossLog` call sites | Physics #4 | SDK-4 |
| **JN-4** | baseproof/trust-network-courts | Adopt explicit Time-of-Receipt asOf (resolve #69 open item) | Physics #1 / Scenario 3 | SDK-1 |
| **E2E** | baseproof/e2e-tests | Cross-stack regression scenarios for the 4 mandates | all | all above |
...
## Relationship to trust-network-courts#69 (PR-C)
...
2. Then **AT-1/2**, **JN-1…4**, **E2E** with `Depends on #<sdk>` links and a
   `Relates to trust-network-courts#69` backref on the JN four.

## Dedup note

`trust-network-courts#69` reviewed directly. The GitHub API was rate-limited during
