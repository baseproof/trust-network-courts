...
> **Depends on:** — (pairs with SDK-1; consumed by trust-network-courts JN-1)
...
> trust-network-courts#69 *assumes* ("asOf = head(seq, RootHash)") but which the
...
| `trust-network-courts/verification/trust/multijurisdiction.go:284-287` | `resolveHead` → `HeadAt(asOf.Sequence)` | consumer drops the root dimension (see JN-1) |
...
- Pairs with **SDK-1**. Consumed by **JN-1**. Realizes the assumed capability in
  **trust-network-courts#69** (Goal 2/14).
