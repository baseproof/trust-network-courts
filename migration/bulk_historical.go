...
DESCRIPTION:
	...
	trust network courts. Uses BuildRootEntity (guide §11.3) and
...
type BulkImportConfig struct {
	...
	// Protects the ledger from admission saturation.
	RateLimit int
...
// : Create case root entities (BuildRootEntity).
// : Create filing amendments under each root (BuildAmendment).
//
...
func RunBulkImport(cfg BulkImportConfig) (*BulkImportResult, error) {
	...
	// assigned log position, which is only known after  submission.
	// This function builds the filing entries with placeholder TargetRoot.
	// The caller submits  first, reads back positions, then
	// patches and submits .
	for _, c := range cfg.Cases {
		for _, f := range c.Filings {
			...
			_, err := builder.BuildAmendment(builder.AmendmentParams{
				...
				SignerDID:   cfg.SignerDID,
				Payload:     payload,
				// TargetRoot: set by caller after  submission
			})
			if err != nil {
				result.Errors = append(result.Errors, BulkImportError{
					DocketNumber: c.DocketNumber,
