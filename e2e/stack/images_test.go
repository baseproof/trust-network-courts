...
//     ghcr.io/baseproof/trust-network-courts.
//
...
func TestResolveImages_Namespaces(t *testing.T) {
	...
	for svc, ref := range relocated {
		...
		if strings.Contains(ref, "attesta-tools") || strings.Contains(ref, "trust-network-courts") {
			t.Errorf("%s image %q must not sit under a legacy or JN-owned namespace", svc, ref)
		}
	}

	own := map[string]string{"aggregator": im.Aggregator, "jn": im.JN}
	for svc, ref := range own {
		if !strings.HasPrefix(ref, "ghcr.io/baseproof/trust-network-courts") {
			t.Errorf("%s image %q: want ghcr.io/baseproof/trust-network-courts* (JN-owned, not relocated)", svc, ref)
		}
