...
func TestBuildArgv_FullWithSecretAndArgs(t *testing.T) {
	got := joined(buildArgv(BuildSpec{
		Tag:        "ghcr.io/baseproof/trust-network-courts:latest",
	...
	for _, want := range []string{
		"docker build -f /repo/deployment/local/Dockerfile.network-api -t ghcr.io/baseproof/trust-network-courts:latest",
