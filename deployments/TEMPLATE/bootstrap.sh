...
#
# Reads config/ YAML files and calls the trust-network-courts CLI to:
...
#
# Prerequisites:
#   - trust-network-courts CLI built and on PATH
...
trust-network-courts provision \
...
trust-network-courts schema-adopt \
    --court-config "${CONFIG_DIR}/court.yaml" \
    --logs-config "${CONFIG_DIR}/logs.yaml" \
...
if [[ -f "${CONFIG_DIR}/anchor.yaml" ]]; then
    trust-network-courts anchor-register \
        --court-config "${CONFIG_DIR}/court.yaml" \
        --logs-config "${CONFIG_DIR}/logs.yaml" \
...
trust-network-courts witness-configure \
    --court-config "${CONFIG_DIR}/court.yaml" \
    --logs-config "${CONFIG_DIR}/logs.yaml" \
