...
DESCRIPTION:
	...
	judicial-cli — the trust-network-courts domain-aware client for talking
	to a running ledger over HTTP. This binary lives in
	trust-network-courts (not the ledger) because it knows about the
...
KEY ARCHITECTURAL DECISIONS:
  ...
  - Walkthrough docs supply the per-schema JSON shapes, citing the
    trust-network-courts/schemas/ struct file:line. A typo in JSON
...
const usage = `judicial-cli — trust-network-courts domain client
...
For walkthrough usage see:
  trust-network-courts/docs/walkthrough/README.md
`

// Version is the semver string. Default tracks the compatibility
// pin documented in the top-level README:
//
//	trust-network-courts v0.0.1
